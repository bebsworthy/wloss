...
/**
 * Deterministic demo seeding inside the app process: onboards programmatically
 * (the same write the wizard's Start step performs — no wizard driving), then
 * seeds the 7-day [DiarySeeder] week through the REAL repositories in the
 * app's own Koin graph ([org.koin.core.context.GlobalContext] — the
 * Application started it in this process).
 *
 * The single implementation of the WLO demo dataset, shared by two drivers:
 * the M3+ instrumented tests (androidTest compiles against the debug variant;
 * [app.wlo.app.SeedingRobot] aliases this object) and the `just demo` seeder
 * ([DemoSeedReceiver]). Runs against the app's frozen demo clock
 * ([ClockPort] → `FixedClock.DEMO_NOW`), so every seeded timestamp is
 * coherent with every screen.
 */
public object DemoSeed {
    ...
    public fun onboardAndSeedWeek(): SeedResult =
        runBlocking {
            ...
            val templates: List<DietTemplate> = OnboardingTemplates.load(reader = koin().get())
