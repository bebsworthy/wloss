---
id: WLO-0083
title: Implement and validate direction-correct weight-gain forecasting
status: done
theme: weight-core
release: 2
created: 2026-09-15T22:54:39Z
modified: 2026-09-16T01:18:26Z
closed: 2026-09-16T01:18:26Z
revision: 8cdb029da0ef0c62
blocks: []
related: [WLO-0073, WLO-0074, WLO-0080]
---

# Description

Replace the current explicit GAIN_FORECAST_UNAVAILABLE hold with a direction-correct, benchmarked gain forecast. Do not reuse the loss integrator by reversing signs without physiological and product validation.
