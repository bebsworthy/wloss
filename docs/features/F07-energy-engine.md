...
- **Cadence:** continuous background recompute (cheap, incremental); one **weekly check-in ritual** (default Sunday 19:00 — the number a geek checks every Sunday), with an **opt-in daily cadence for power users in v1** (R-A7 — daily proposals remain Apply-only; the weekly report card is unchanged); episodic deep-dives at goal-setting (with F01), plateau, and diet-break moments.
...
- **3-band goal-date forecast with deceleration (the open win):** every competitor extrapolates the historic rate linearly and turns optimistic near goal (Happy Scale's community-observed flaw). WLO's model:
  - **Direction-specific deceleration core:** loss re-estimates BMR at future bodyweight while holding the measured Adjustment and steps baseline. Gain uses the adult linearized dynamic-energy equation: expenditure rises by 22 kcal/day for each kg gained, so a fixed surplus progressively shrinks and a target beyond its equilibrium receives no date. Gain is a distinct model path, not sign-reversed loss. Both directions remain **partition-free** (R-A6), use the published 7,700 kcal/kg product approximation, and name that approximation in provenance. Evidence and limitations are published in [`weight-gain-forecast-v1.md`](../research/weight-gain-forecast-v1.md).
...
- **Shared concepts:** Targets (write authority split: **F01 owns initial targets; F07 owns adaptive adjustments; both write through the same auditable, user-confirmed path** — no other feature may write calorie/macro targets; the schema itself is ratified in FEATURES.md Appendix A, and Apply is its F07 writer); Trend weight (computed in F06 only); Provenance; Day statuses (Logged/Skipped/Fasted — captured in F02's day header, consumed here).
...
- **Energy partitioning:** should the forecast assume fat-loss fractions (lean-mass preservation from F05 training context) when projecting bodyweight change — more honest, but more assumptions to publish. *(Resolved: R-A6 — partition-free in v1; composition-aware rates are [v1.x].)*
- **Check-in cadence choice:** weekly frozen (ritual value, Zolt/MF precedent); should power users unlock daily cadence, and does that dilute the ritual? *(Resolved: R-A7 — weekly default, opt-in daily ships in v1; report card stays weekly.)*
