...
- **Headline pair:** raw weigh-in (small) + canonical trend weight (hero) + weekly rate ("−0.6 kg/wk"). Owner amendment WLO-0104: the page shows neutral change between the first and last available trend points in the selected chart period, with their dates. The canonical trend remains the single headline answer; the 30-day value has no goal coloring or alternate-number status.
...
WLO-0104 implementation contract: see FEATURES.md weight overview amendment and docs/design/mockups/WLO-0103/README.md for page hierarchy, raw dots, goal visibility, tooltip inspection and off-page settings. Chart ranges crop one full-history trend.
