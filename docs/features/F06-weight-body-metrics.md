...
- **The logbook:** a raw table of every entry with provenance column, inline edit, and delete (R-B8 amendment) — the geek's ground truth beneath every smoothed view, and the source of the import log (which device wrote what, when, under which dedup rule). The delete door is the stock M3 swipe-to-dismiss on each row (WLO-0050): the item slides as one opaque piece, and the delete action — an error-tinted glyph + label, never a color fill — is revealed in the space it vacates; a full swipe past the component threshold fires the delete from `confirmValueChange` (the deliberate gesture is the confirmation), partial swipes spring back, and the undo notice renders inline where the row was — never a dialog, never a page-level banner, never an overlap.
