...
| Swipe-to-dismiss list row | F06 (WLO-0050; M3 `SwipeToDismissBox`) | settled / swiping (action revealed in the vacated space) / dismissed | [v1] | Stock M3 SwipeToDismissBox, `enableDismissFromStartToEnd = false`; the full swipe past the component threshold fires the action from `confirmValueChange` — the deliberate gesture is the confirmation; partial swipes spring back. The item content is opaque in the card's own color, so it slides as one piece and masks the action until its space is vacated — never an overlap, never a color fill; the revealed action is an error-tinted glyph + label at the trailing edge. The undo renders inline where the row was (never a dialog, never a page-level banner). TalkBack gets a custom row action |
