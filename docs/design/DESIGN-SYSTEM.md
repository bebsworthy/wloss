...
Hero number card (trend-first; flips post-weigh-in; yields to check-in card on check-in day R-U17; night = minimal) · Delta chip (150 ms pop; never alarm-red) · Calories-remaining ring (adaptive; macro dots; "provisional" fallback when held; "the ring shows *state*, not *sin*") · Meals card ("Meals · today / tomorrow"; next-open-meal row + header count; confirmed meals live only in the diary, R-D13; "log as planned"; muted-by-choice) · Quick-action rail (photo/weigh/gut/workout; badge-dots; 32 ms press; long-press camera = kcal-only; re-ranked by Adaptive Day Model) · Weigh-in card (leads mornings until logged) · F07 check-in card slot (promoted to hero on check-in day, R-U17) · "Close the day" / "Plan tomorrow" cards (~19:00; "copy today" preset) · Streak chip · Report-card-ready pill · Backup-health dot · Collapsed-card chip (swipe-up fold; 5 s undo) · Day-closure recap [v1.x] ("Day closed"; 3 stat chips + share; R-U9 in-app first) · Home-screen widget (4×2 hero + ring; every zone deep-links; lock-screen redaction to dots; interactive [v1.x]) · Nudge notification (silent channel; ≤ 2 action buttons; animated collapse; "Why am I seeing this?" + inline tuning; overflow "Pause nudges for a week") · Wear OS glance [future].
