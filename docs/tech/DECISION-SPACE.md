...
| ID | Decision | Candidates / considerations |
|---|---|---|
| T-E1 | **Inference runtime** | LiteRT (TFLite) vs ONNX Runtime Mobile vs MediaPipe Tasks vs ExecuTorch. Constraints: clean under the C5 audit-card filter (license + data-flow), NPU/GPU delegates on mid-range, model-zoo diversity (detector, segmenter, pose, OCR, STT, embeddings, LLM). Note: MediaPipe Tasks *library* is Apache-2.0 — its prebuilt `.so`/`.task` artifacts just need the standard audit card now; "no third-party SDK in the silhouette module" (F08 §9) may still push silhouette to raw TFLite models. ▸ research a runtime matrix per capability; likely LiteRT core + selective MediaPipe Tasks. |
| T-E2 | **Food-photo model** | Detector + class head, 20–40 MB bundled, energy-density sanity rails (F02 §3). Research: candidate architectures (YOLO-nano class), open datasets/weights with named licensing (R-S12), on-device benchmark on min-spec device, quantization (int8) quality loss. Biggest *research* risk in the zoo. |
